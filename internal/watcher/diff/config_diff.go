...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	if len(oldCfg.InteractionsKey) != len(newCfg.InteractionsKey) {
		...
		for i := range oldCfg.InteractionsKey {
			...
			if !reflect.DeepEqual(o.DisableCooling, n.DisableCooling) {
				changes = append(changes, fmt.Sprintf("interactions[%d].disable-cooling: %s -> %s", i, formatOptionalBool(o.DisableCooling), formatOptionalBool(n.DisableCooling)))
			}
			if !reflect.DeepEqual(o.RequestRetry, n.RequestRetry) {
				changes = append(changes, fmt.Sprintf("interactions[%d].request-retry: %s -> %s", i, formatOptionalInt(o.RequestRetry), formatOptionalInt(n.RequestRetry)))
			}
			if strings.TrimSpace(o.APIKey) != strings.TrimSpace(n.APIKey) {
