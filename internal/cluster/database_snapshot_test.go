...
func TestDatabaseSnapshotSQLiteRoundTrip(t *testing.T) {
	...
	assertDatabaseSnapshotResultCounts(t, result, map[string]DatabaseSnapshotImportTableResult{
		...
		"in_flight_lease": {Name: "in_flight_lease", Skipped: 1},
...
func TestDatabaseSnapshotImportRejectsNonEmptyTarget(t *testing.T) {
	...
	if errCreate := target.Create(&ConfigRecord{Key: "existing", Value: JSONB(`{"value":true}`), Version: 1}).Error; errCreate != nil {
	...
	var count int64
	if errCount := target.Model(&ConfigRecord{}).Count(&count).Error; errCount != nil || count != 1 {
		t.Fatalf("target config count = %d, error = %v", count, errCount)
	}
}
...
func seedDatabaseSnapshotTestData(t *testing.T, db *gorm.DB) databaseSnapshotTestData {
	...
	records := []any{
		...
		&InFlightLeaseRecord{ID: 801, LeaseID: "lease-1", DispatchID: "dispatch-1", CredentialID: authUUID, Model: "gpt-test", Status: InFlightLeaseStatusActive, StartedAt: now, LastRenewedAt: now, ExpiresAt: expiresAt, CreatedAt: now, UpdatedAt: now},
