...
func TestInFlightLimitsFromConfig(t *testing.T) {
	limits, errLimits := InFlightLimitsFromConfig(config.DefaultCredentialInFlightConfig())
	if errLimits != nil {
		t.Fatalf("InFlightLimitsFromConfig() error = %v", errLimits)
	}
	if limits.MaxPartBytes != config.DefaultInFlightMaxPartBytes || limits.MaxRevisionBytes != config.DefaultInFlightMaxRevisionBytes || limits.MaxStringBytes != config.DefaultInFlightMaxStringBytes {
