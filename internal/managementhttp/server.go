...
func registerClusterManagementRoutes(r *RouteRegistry, handler *clustermanagement.Handler) {
	...
	r.Set(http.MethodGet, "/credentials/in-flight", handler.ListCredentialInFlight)
	r.Set(http.MethodGet, "/credentials/in-flight/summary", handler.GetCredentialInFlightSummary)
	r.Set(http.MethodGet, "/credentials/:credential_id/in-flight", handler.GetCredentialInFlight)
