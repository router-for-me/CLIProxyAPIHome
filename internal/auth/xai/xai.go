...
func BuildAuthorizeURL(params AuthorizeURLParams) (string, error) {
	endpoint, errEndpoint := ValidateOAuthEndpoint(params.AuthorizationEndpoint, "authorization_endpoint")
	if errEndpoint != nil {
		return "", errEndpoint
	}
...
func (a *XAIAuth) Discover(ctx context.Context) (*Discovery, error) {
	...
	authorizationEndpoint, errAuthorization := ValidateOAuthEndpoint(payload.AuthorizationEndpoint, "authorization_endpoint")
	if errAuthorization != nil {
		return nil, errAuthorization
	}
	tokenEndpoint, errToken := ValidateOAuthEndpoint(payload.TokenEndpoint, "token_endpoint")
	if errToken != nil {
		return nil, errToken
	}
...
func (a *XAIAuth) ExchangeCodeForTokens(ctx context.Context, code, redirectURI string, pkceCodes *PKCECodes, tokenEndpoint string) (*AuthBundle, error) {
	...
	tokenData, errToken := a.postTokenForm(ctx, tokenEndpoint, form)
	if errToken != nil {
		return nil, errToken
	}
	return &AuthBundle{
