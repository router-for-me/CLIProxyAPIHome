...
`GET /credentials/in-flight` returns bounded request details derived only from the latest complete CPA snapshots. It accepts optional `credential_id` and `model` exact-match filters plus `limit` and `offset`. `limit` defaults to 100 and is bounded to 1 through 1000; `offset` defaults to 0. Each item contains `request_id`, `credential_id`, normalized upstream `model`, `request_kind`, and `started_at`. `request_id` is a display and correlation field, not a row-uniqueness guarantee; clients must preserve distinct returned rows even when request IDs repeat. A credential with a policy also includes its authoritative `limiter` state. It never exposes CPA identity, tokens, or request payloads. Every response also returns `total`, nullable `next_offset`, nullable `snapshot_cursor`, and nullable `snapshot_expires_at`. When `offset + returned item count < total`, `next_offset` equals that sum; otherwise `next_offset` is `null`.

Clients with `capabilities.credential_in_flight_snapshot_cursor=true` can request stable multi-page reads. Send `stable_snapshot=true` on the first page. When another page exists, Home stores the filtered observation and joined limiter states as a short-lived immutable database view, then returns an opaque `snapshot_cursor` and RFC 3339 `snapshot_expires_at`. The cursor remains valid for 2 minutes and can be read by any Home instance sharing the database. Every later page must send the same `credential_id` and `model` values plus that `snapshot_cursor`; `offset` should use the preceding `next_offset`, while `limit` may change. Clients must also keep `total` and `snapshot_expires_at` identical across all pages. The cursor is omitted when the first response has no next page. Items and limiter values remain pinned to the stored view even if a newer CPA snapshot is ingested. Freshness diagnostics are not frozen: their deadline remains based on the original CPA snapshots' Home database ingestion timestamps, so creating a cursor never grants a new freshness interval. A page read after that deadline returns `stale=true` and `coverage_complete=false`.

An invalid, expired, or filter-mismatched cursor returns HTTP `409` with `error: "in_flight_snapshot_cursor_expired"`; clients must discard all accumulated pages and restart from offset 0. Failure to persist a first-page cursor returns HTTP `500` with `error: "in_flight_snapshot_cursor_store_failed"`. Cursor payloads have a 64 MiB hard limit. Failure to load or decode a stored cursor returns HTTP `500` with `error: "in_flight_snapshot_cursor_load_failed"`. Expired cursor rows are removed opportunistically when new cursors are created. Cursor rows are transient migration-only data: the table is auto-migrated for runtime use but is excluded from database snapshot export and import.
