...
`reset_credits` is `null` when the provider does not report this capability or no observation has been collected. It is detail-only and is not included in `GET /quota/credentials` list items. `available_count` is the provider-reported number of currently available credits, `observed_at` is the time of the reset-credit observation, and `credits` contains the bounded, expiry-sorted list of currently available Codex rate-limit reset credits. Credit entries expose only status and timing metadata; provider reset-credit identifiers are not returned. This endpoint is read-only and does not consume a credit.
